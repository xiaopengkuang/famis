        ...
        columns: [[
            ...
            { field: 'Method_add', title: '添加方式', width: 50 },
            {
                ...
                formatter: function (data) {
                    if (data == "维修") {
                        return '<font color="#00FF00">' + data + '</font>';
                    }
                    else if (data == "报废") {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data > 0) {
                try {
                    parent.loadInitData();
                    parent.updateCounterInfo();
                    parent.$("#modalwindow").window("close");
                } catch (e) {
                    $.messager.alert('警告', "自动关闭页面失败，请手动关闭该页面！", 'warning');
                }

            } else {
                if (data == -2) {
                    $.messager.alert('警告', "请确认添加资产明细或者检查所有资产均为非报废状态！", 'warning');
                } else {
                    $.messager.alert('警告', "请确认添加资产明细或者检查所有资产均为非报废状态！", 'warning');
                }
            }
